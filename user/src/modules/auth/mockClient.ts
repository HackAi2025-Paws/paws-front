...
const STORAGE_KEY = env.SESSION_STORAGE_KEY
...
function saveSession(session: AuthSession) {
  ...
  // También guardar el token por separado
  if (session.token) {
    localStorage.setItem(env.AUTH_TOKEN_KEY, session.token)
  }
}
...
phoneToUserMap.set('+5491123456789', 'demo@paws.com')
phoneToUserMap.set('+5491187654321', 'maria@example.com')
phoneToUserMap.set('+5491134567890', 'juan@example.com')
...
export const mockAuthClient: AuthClient = {
  ...
  async register(input: RegisterInput): Promise<AuthSession> {
    ...
    usersDb.set(email, user)
    // Also map phone to email for OTP login
    phoneToUserMap.set(phone, email)
